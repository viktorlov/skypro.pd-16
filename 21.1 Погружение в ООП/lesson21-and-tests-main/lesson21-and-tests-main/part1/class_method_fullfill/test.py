...
class ClassesTestCase(SkyproTestCase):
    ...
    def test_init_method(self):
        ...
        self.assertTrue(
            ...
            "%@Проверьте, что после использования метода fullfill "
            "количество товаров в экземпляре класса увеличилось на сумму, равную "
            "числу товаров, находящихся на складе."
        )
        self.assertTrue(
            self.storage.goods_quantity == 0,
            "%@Проверьте, что после использования метода fullfill значение находящихся на складе (в классе) "
            "равно нулю"
        )
